...
def write_parameters(l):
    home_dir = str(pathlib.Path().absolute())
    with open(home_dir + "/outputs/"+"_".join(l.stations) + "_" + str(l.freq[0]) + "-" + str(l.freq[1])+"Hz" + "params.txt", 'w') as f:
        print(l.__dict__, file=f)
...
def get_files(l):
    ...
    files.sort()
    start_date = l.detection_times[0][0].strftime("%Y-%m-%d")
    start_index = [s for s in range(len(files)) if start_date in files[s]][0]
    end_date = l.detection_times[-1][1].strftime("%Y-%m-%d")
...
def get_detections_today(l):
    ...
    bool_indices = np.logical_and(l.detection_times[:,1]>=current_date,l.detection_times[:,1]<current_date + timedelta(days=1))
